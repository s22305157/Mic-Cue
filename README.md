...
語音選單中的「台灣 AI 男聲 — 雲哲」使用公開的 `tts.kina.ink` HTTP TTS 服務與 Microsoft Edge 線上語音 `zh-TW-YunJheNeural`。它不依賴 iPhone 的系統語音，但需要網路；播放時台詞會傳送到第三方服務。該服務沒有可用性保證，若暫時中斷，請改用裝置語音。
